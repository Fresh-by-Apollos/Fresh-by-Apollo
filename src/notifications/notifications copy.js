...
async function loadUsers(usersRef) {
  let messages = [];
  usersRef.onSnapshot(async function (collection) {
    console.log("collection:", collection);
    collection.forEach(async (user) => {
      let newMessages = await loadUser(user);
...
async function loadUser(user) {
  let token = await user.data().expoPushToken;
  if (token) {
    ...
    let messages = await loadFridgeItems(fridgeRef, user, token);
    return messages;
  }
}

async function loadFridgeItems(fridgeRef, user, token) {
  console.log('entered loadFridgeItems')
  let messages = [];
  fridgeRef.onSnapshot(async function (fridgeSnapshot) {
    console.log('entered fridgeRef.onSnapshot')
    fridgeSnapshot.forEach(async (fridgeItem) => {
      let newMessages = await loadFridgeItem(fridgeItem, user, token);
      messages = [...messages, ...newMessages];
    });
...
async function loadFridgeItem(fridgeItem, user, token) {
  console.log('entered loadFridgeItem')
...
cron.schedule("*/1 * * * *", async function loadPushNotifications() {
  ...
  let messages = await loadUsers(usersRef);
